function clusnames = plot_mahalanobis(in, temp_size, start_offset, pulse_clus,comps, sigma,plotting_vis)
...
for a = 1:length(clusnames) % for each cluster, plot distance from center and scores through time
    ...
    if plotting_vis % plot the visual cluster, first two components specified in comps[]
        ...
        title(sprintf('Cluster %d: X axis: Component %d score Y axis: Component %d score',clusnames(a),comps(1),comps(2)),'Fontsize',5);
        set(gca,'XTick',[],'YTick',[]);
        xlabel(''); ylabel('');
        cur_plot = cur_plot+1;
    end
    padded_time = clus{a}.times(1):clus{a}.times(end); % add entries for all intermediate times, for smoothing
    padded_mahal = zeros(1,length(padded_time)); % add zeros for smoothing
    %% Because of the rounding to the closest millisecond, clus{a}.times does not necessarily have unique timepoints
    %% For now, the fix is to discard these distances from analysis, must come up with a better way.
    mahaldistances = clus{a}.mahal_distance;
    mahaldistances(diff(clus{a}.times) == 0) = [];
    padded_mahal(ismember(padded_time,clus{a}.times)) = mahaldistances;
    ...
    h(cur_plot) = plot(clus{a}.times, clus{a}.mahal_distance, 'k.','MarkerSize',5);
        title(['X axis: Time (ms) Y axis: Mahalanobis Distance using Components ' sprintf('%d ',comps')],'Fontsize',5);
        set(gca,'XTick',[],'YTick',[]);
    hold on;
    plot(padded_time, sdf, 'r-','LineWidth',2);
    cur_plot = cur_plot+1;
    ...
    %% For now, the fix is to discard these distances from analysis, must come up with a better way.
        ...
        these_scores(diff(clus{a}.times) == 0) = [];
        ...
        plot(padded_time, sdf, 'r-','LineWidth',2);
        title(sprintf('X axis: Time (ms) Y axis: Component %d score',comps(c)),'Fontsize',5);
        set(gca,'XTick',[],'YTick',[]);
        cur_plot = cur_plot+1;
    end
    %figure();
